...
- `score_pair` in `src/scoring.py` returns `tuple[LLMScoreOutput, CallMetadata]`, not just `LLMScoreOutput`. Any code that calls `score_pair` must unpack both values. The `CallMetadata` object contains tokens, latency, and cost for the call. Do not discard it silently. Callers that don't need it for aggregation should still be updated when the signature is modified. See `src/pipeline.py` and `scripts/run_eval.py` for the established pattern.
...
**CI.** `.github/workflows/ci.yml` runs on push and PR: install dependencies, `ruff check`, `pytest`, and `docker build` to verify the image builds. No pipeline execution in CI (no AWS credentials, no point). `ruff check .` is configured in `pyproject.toml` and must pass locally before committing. E402 (module-level import not at top) is globally ignored because all scripts use `sys.path.insert` before imports — this is intentional. `pandas` is now an explicit dependency in `requirements.txt` (used by the Run Metadata tab).

**Bedrock invocation logging is off by default** per region per account. Turning it on is a manual step in the Bedrock console (Settings → Model invocation logging). Without it, no Bedrock usage metrics will publish to CloudWatch. This project instruments at the application layer instead — `CallMetadata` in `src/scoring.py` captures tokens/latency/cost at the API response level and does not depend on AWS-side logging. See `DECISIONS.md` entry dated 2026-05-05 for rationale.
